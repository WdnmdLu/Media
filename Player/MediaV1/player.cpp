...
void Player::videoDelay(AVFrame* frame){
    ...
    if(LastVideoTimeStamp == 0.0f){
        // 此时这个是第一帧，直接进行显示不delay
        LastVideoTimeStamp = CurrentVideoTimeStamp;
        return;
    }
    // 计算的是当前的这一帧与音频时间的差值，转化为秒
    int diff = (LastVideoTimeStamp - AudioTimeStamp)*1000;
    // 计算delay也就是当前这一帧要显示多长时间
    int64_t delay = (int64_t)((CurrentVideoTimeStamp - LastVideoTimeStamp)*1000);
